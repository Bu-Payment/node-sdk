...
describe("CustomersResource", () => {
  it("lists customers by email", async () => {
    ...
    expect(pathOf(calls[0] as never)).toBe("/v1/customers");
    expect(queryOf(calls[0] as never)).toBe("?email=buyer%40example.test&limit=10");
  });
  ...
  it("walks every customer page", async () => {
    ...
    expect(queryOf(calls[1] as never)).toBe("?email=buyer%40example.test&cursor=cur_2");
  });

  it("reads one customer", async () => {
    const { client, calls } = harnessReturning({ id: "cus_1" });
    await client.customers.get("cus_1");
    expect(pathOf(calls[0] as never)).toBe("/v1/customers/cus_1");
  });
  ...
  it("creates a customer with a generated idempotency key", async () => {
    ...
    expect(calls[0]?.body).toEqual({ email: "buyer@example.test", name: "Buyer" });
