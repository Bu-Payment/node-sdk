...
describe("checkout resources", () => {
  it("evaluates a coupon without an idempotency key", async () => {
    const { client, calls } = harnessReturning({ valid: true });
    await client.coupons.evaluate({ code: "WELCOME", unitAmount: 1000, currency: "EUR" });
    expect(pathOf(calls[0] as never)).toBe("/v1/coupons/evaluate");
    expect(calls[0]?.body).toEqual({ code: "WELCOME", unitAmount: 1000, currency: "EUR" });
  ...
  it("redeems a coupon against a reference", async () => {
    ...
    expect(pathOf(calls[0] as never)).toBe("/v1/coupons/redeem");
  ...
  it("lists the tax rates applicable to a product", async () => {
    ...
    expect(pathOf(calls[0] as never)).toBe("/v1/tax-rates");
    expect(queryOf(calls[0] as never)).toBe("?productId=prod_1&country=PT");
  });

  it("calculates tax on one rate", async () => {
    const { client, calls } = harnessReturning({ taxRateId: "txr_1" });
    await client.taxRates.calculate("txr_1", { amount: 1000, productId: "prod_1" });
    expect(pathOf(calls[0] as never)).toBe("/v1/tax-rates/txr_1/calculate");
    expect(calls[0]?.body).toEqual({ amount: 1000, productId: "prod_1" });
  });
  ...
  it("walks every shipping rate page keeping the resolved destination", async () => {
    ...
    expect(queryOf(calls[1] as never)).toBe(
  ...
  it("opens a subscription checkout session", async () => {
    ...
    expect(pathOf(calls[0] as never)).toBe("/v1/subscription-checkouts");
