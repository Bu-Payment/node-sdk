...
describe("PaymentsResource", () => {
  ...
  it("charges an ad hoc amount when no canonical price applies", async () => {
    ...
    expect(callAt(calls, 0).body).toEqual({ customerId: "cus_1", amount: 1500, currency: "EUR" });
  ...
  it("refuses a currency that would override the canonical price", async () => {
    const { client, calls } = harnessReturning({ id: "pay_1" });
    const body = { customerId: "cus_1", priceId: "price_1", currency: "USD" } as CreatePaymentBody;
    await expect(client.payments.create(body)).rejects.toMatchObject({
      code: ErrorCode.REQUEST_INVALID,
    });
    expect(calls).toHaveLength(0);
  ...
  it("refuses a currency with no amount beside it", async () => {
    const { client, calls } = harnessReturning({});
    const body = { customerId: "cus_1", currency: "EUR" } as CreatePaymentBody;
    await expect(client.payments.create(body)).rejects.toMatchObject({
      code: ErrorCode.REQUEST_INVALID,
    });
    expect(calls).toHaveLength(0);
